...
const showInputError = (formElement, inputElement, errorMessage, enableValidationParams) => {
  ...
  const isValid = (formElement, inputElement) => {
    if (!inputElement.validity.valid) {
      ...
      showInputError(formElement, inputElement, inputElement.validationMessage,enableValidationParams);
    } else {
  ...
  const setEventListeners = (formElement,enableValidationParams) => {
    ...
    const inputList = Array.from(formElement.querySelectorAll(enableValidationParams.inputSelector));
    const buttonElement = formElement.querySelector(enableValidationParams.submitButtonSelector);
  ...
  const enableValidation = (enableValidationParams) => {
    ...
    // сделаем из них массив методом Array.from
    const formList = Array.from(document.querySelectorAll(enableValidationParams.formSelector));
  ...
  const toggleButtonState = (inputList, buttonElement,enableValidationParams) => {
